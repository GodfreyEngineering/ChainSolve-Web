...
- [ ] **1.12 Stripe webhook idempotency audit** — Verify `stripe_events` table dedup works: INSERT ON CONFLICT DO NOTHING on `id` (Stripe event ID). Check all webhook handlers check for existing event before processing. Verify: replay same webhook event — no duplicate processing.
