...
export async function createRun(input: CreateRunInput): Promise<ExperimentRun> {
  ...
  const { data, error } = await supabase.from('experiment_runs').insert(row).select().single()
...
export async function updateRun(id: string, input: UpdateRunInput): Promise<void> {
  ...
  const { error } = await supabase.from('experiment_runs').update(updates).eq('id', id)
 ...
 */
export async function getRunsForProject(projectId: string, limit = 100): Promise<ExperimentRun[]> {
...
export async function deleteRun(id: string): Promise<void> {
  const { error } = await supabase.from('experiment_runs').delete().eq('id', id)
...
export async function saveCheckpoint(
  ...
  const { error } = await supabase.from('experiment_checkpoints').insert({
    run_id: runId,
    epoch,
    metrics,
