    ...
    <AppWindow windowId={AI_COPILOT_WINDOW_ID} title={t('ai.title')} minWidth={400} minHeight={360}>
      <div style={s.container}>
        ...
        <div style={s.tabBar}>
          ...
          <button
            style={s.tab(task === 'generate_theme')}
            onClick={() => setTask('generate_theme')}
          >
        ...
        <div ref={transcriptRef} style={s.transcript}>
          {messages.length === 0 && (
            <div
              style={{ opacity: 0.4, textAlign: 'center', padding: '1rem 0', fontSize: '0.8rem' }}
            >
              {task === 'fix_graph'
          ...
          {loading && (
            <div style={{ ...s.msgAi, opacity: 0.6, fontStyle: 'italic' }}>
              {t('ai.generating')}
            </div>
          )}
        </div>
