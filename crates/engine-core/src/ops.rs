...
fn evaluate_node_inner(
    ...
    match block_type {
        ...
        "fmu.export" => {
            // Generate a minimal FMI 2.0 modelDescription.xml based on the
            // connected input value. The XML can be downloaded by the user and
            // packaged into a .fmu archive (ZIP) with compiled C stubs.
            let model_name = data.get("fmuModelName")
                .and_then(|v| v.as_str())
                .unwrap_or("ChainSolveFMU");
            let description = data.get("fmuDescription")
                .and_then(|v| v.as_str())
                .unwrap_or("Generated by ChainSolve");
            let guid = data.get("fmuGuid")
                .and_then(|v| v.as_str())
                .unwrap_or("{00000000-0000-0000-0000-000000000000}");

            // Determine output dimension from input value
            let (n_outputs, sample_values) = match inputs.get("data") {
                Some(Value::Scalar { value }) => (1usize, vec![*value]),
                Some(Value::Vector { value, .. }) => (value.len(), value.clone()),
                Some(Value::Table { columns, rows }) => {
                    let n = columns.len();
                    let first_row = rows.first().cloned().unwrap_or_else(|| vec![0.0; n]);
                    (n, first_row)
                }
                _ => (1, vec![0.0]),
            };

            // Generate ScalarVariable entries for each output
            let mut vars_xml = String::new();
            for i in 0..n_outputs {
                let start = sample_values.get(i).copied().unwrap_or(0.0);
                vars_xml.push_str(&format!(
                    "    <ScalarVariable name=\"output_{i}\" valueReference=\"{i}\" causality=\"output\" variability=\"continuous\">\n      <Real start=\"{start:.6}\"/>\n    </ScalarVariable>\n"
                ));
            }
...
{vars_xml}  </ModelVariables>
  <ModelStructure>
    <Outputs>
{outputs_refs}    </Outputs>
  </ModelStructure>
</fmiModelDescription>"#,
                outputs_refs = (0..n_outputs)
                    .map(|i| format!("      <Unknown index=\"{}\"/>\n", i + 1))
                    .collect::<String>()
            );

            Value::Text { value: xml }
        }
