...
export const onRequestPost: PagesFunction<{
  ...
  const { STRIPE_SECRET_KEY, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY } =
    context.env;
  ...
  // ── Build account link ───────────────────────────────────────────────────
  const origin =
    context.request.headers.get("Origin") ?? "https://chainsolve.com";

  const accountLink = await stripe.accountLinks.create({
    account: accountId,
    refresh_url: `${origin}/marketplace/author?connect=refresh`,
    return_url: `${origin}/marketplace/author?connect=return`,
