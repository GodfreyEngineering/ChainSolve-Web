-- ============================================================
-- ChainSolve — Consolidated Baseline Schema
--
-- Single migration for a fresh Supabase project.
-- Consolidates archived migrations 0001–0012 into one file.
--
-- Run via:  supabase db push   /   supabase db reset
-- Or paste into: Supabase Dashboard > SQL Editor > Run
--
-- Prerequisites:
--   - Supabase project with auth.users enabled
--   - Service-role key set in .env (for edge functions)
--
-- For manual disaster-recovery, see: supabase/scripts/bootstrap.sql
-- ============================================================
...
-- 2h. stripe_events (service_role only)
...
-- 2m. observability_events (service_role only)
...
COMMENT ON TABLE  public.user_sessions IS 'Tracks active browser sessions for device management.';
COMMENT ON COLUMN public.user_sessions.device_label IS 'Human-readable device description parsed from User-Agent.';
COMMENT ON COLUMN public.user_sessions.last_active_at IS 'Last time this session was actively used.';
...
COMMENT ON TABLE public.user_terms_log IS 'Immutable audit log of every ToS acceptance.';
...
-- 3a. Canonical updated_at trigger function (single function for all tables)
...
-- 3b. Auto-create profile + preferences on signup
...
-- 3d. Project limit enforcement (final version from 0011)
...
-- 3e. Canvas limit enforcement (from 0010)
...
-- 3f. Marketplace counter functions
CREATE OR REPLACE FUNCTION public.handle_marketplace_likes_count()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE public.marketplace_items SET likes_count = likes_count + 1 WHERE id = NEW.item_id;
    RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    UPDATE public.marketplace_items SET likes_count = GREATEST(likes_count - 1, 0) WHERE id = OLD.item_id;
    RETURN OLD;
  END IF;
  RETURN NULL;
END;
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE public.marketplace_items SET downloads_count = downloads_count + 1 WHERE id = NEW.item_id;
    RETURN NEW;
  END IF;
  RETURN NULL;
END;
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    UPDATE public.marketplace_items SET comments_count = comments_count + 1 WHERE id = NEW.item_id;
    RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    UPDATE public.marketplace_items SET comments_count = GREATEST(comments_count - 1, 0) WHERE id = OLD.item_id;
    RETURN OLD;
  END IF;
  RETURN NULL;
END;
...
-- 3g. Marketplace official flag auto-set
...
-- 3h. Rate limit + org policy enforcement (with pinned search_path from 0003)
...
-- 3i. Audit log retention cleanup
...
-- 3j. Developer auto-flag trigger function (from 0009)
...
-- updated_at triggers (all use canonical trigger_set_updated_at)
...
CREATE TRIGGER set_updated_at BEFORE UPDATE ON public.fs_items
  FOR EACH ROW EXECUTE FUNCTION public.trigger_set_updated_at();
...
CREATE TRIGGER set_ai_org_policies_updated_at BEFORE UPDATE ON public.ai_org_policies
  FOR EACH ROW EXECUTE FUNCTION public.trigger_set_updated_at();

DROP TRIGGER IF EXISTS set_ai_usage_monthly_updated_at ON public.ai_usage_monthly;
CREATE TRIGGER set_ai_usage_monthly_updated_at BEFORE UPDATE ON public.ai_usage_monthly
  FOR EACH ROW EXECUTE FUNCTION public.trigger_set_updated_at();
...
-- Marketplace counter triggers
DROP TRIGGER IF EXISTS trg_mkt_likes_count ON public.marketplace_likes;
CREATE TRIGGER trg_mkt_likes_count AFTER INSERT OR DELETE ON public.marketplace_likes
...
CREATE TRIGGER trg_mkt_downloads_count AFTER INSERT ON public.marketplace_purchases
...
CREATE TRIGGER trg_mkt_comments_count AFTER INSERT OR DELETE ON public.marketplace_comments
  FOR EACH ROW EXECUTE FUNCTION public.handle_marketplace_comments_count();

-- Marketplace auto-official
DROP TRIGGER IF EXISTS trg_mkt_items_official ON public.marketplace_items;
CREATE TRIGGER trg_mkt_items_official BEFORE INSERT ON public.marketplace_items
  FOR EACH ROW EXECUTE FUNCTION public.handle_marketplace_item_official();

-- Rate limit + org policy enforcement
DROP TRIGGER IF EXISTS trg_comment_rate_limit ON public.marketplace_comments;
CREATE TRIGGER trg_comment_rate_limit BEFORE INSERT ON public.marketplace_comments
  FOR EACH ROW EXECUTE FUNCTION public.enforce_comment_rate_limit();

DROP TRIGGER IF EXISTS trg_enforce_org_install_policy ON public.marketplace_install_events;
CREATE TRIGGER trg_enforce_org_install_policy BEFORE INSERT ON public.marketplace_install_events
...
CREATE TRIGGER trg_enforce_org_comment_policy BEFORE INSERT ON public.marketplace_comments
...
-- Final version from 0010: prevents self-promotion of ALL role flags
...
-- 6i. csp_reports
CREATE POLICY csp_reports_insert_own ON public.csp_reports
  FOR INSERT TO authenticated
  WITH CHECK (user_id = (select auth.uid()));

-- 6j. observability_events (deny-all for authenticated)
...
CREATE POLICY mkt_items_author_insert ON public.marketplace_items
  FOR INSERT TO authenticated
  WITH CHECK (
    author_id = (select auth.uid())
    AND EXISTS (
      SELECT 1 FROM public.profiles
      WHERE id = (select auth.uid()) AND verified_author = true
    )
  );
...
CREATE POLICY mkt_items_author_delete ON public.marketplace_items
  FOR DELETE TO authenticated
  USING (author_id = (select auth.uid()));

-- 6m. marketplace_purchases
CREATE POLICY mkt_purchases_user_select ON public.marketplace_purchases
  FOR SELECT TO authenticated
  USING (user_id = (select auth.uid()));

CREATE POLICY mkt_purchases_user_insert ON public.marketplace_purchases
  FOR INSERT TO authenticated
  WITH CHECK (user_id = (select auth.uid()));
...
-- 6p. marketplace_comments (consolidated from 0005)
...
CREATE POLICY org_members_select ON public.org_members
  ...
  USING (
    EXISTS (
      ...
      WHERE m2.org_id = org_members.org_id AND m2.user_id = (select auth.uid())
    )
  );
...
CREATE POLICY org_members_update ON public.org_members
  ...
  USING (
    EXISTS (
      ...
      WHERE m.org_id = org_members.org_id
        AND m.user_id = (select auth.uid())
        AND (m.role = 'owner' OR (m.role = 'admin' AND org_members.role = 'member'))
    )
  )
  WITH CHECK (
    EXISTS (
...
CREATE POLICY student_verif_select_own ON public.student_verifications
  FOR SELECT TO authenticated
  USING (user_id = (select auth.uid()));
...
VALUES
  ('projects',    'projects',    false, 52428800),  -- 50 MB, private
  ('uploads',     'uploads',     false, 52428800),  -- 50 MB, private
  ('marketplace', 'marketplace', true,  10485760)   -- 10 MB, public
ON CONFLICT (id) DO NOTHING;
...
CREATE POLICY "projects bucket: users insert own files"
  ...
  WITH CHECK (
    ...
    AND public.user_can_write_projects((select auth.uid()))
  );
...
CREATE POLICY "uploads bucket: users insert own files"
  ...
  WITH CHECK (
    bucket_id = 'uploads'
    AND (storage.foldername(name))[1] = (select auth.uid())::text
    AND public.user_has_active_plan((select auth.uid()))
  );
...
-- marketplace bucket (public-read, author-write)
...
-- =========================================================================
-- SCALING NOTES (for production with 1000s of concurrent users)
-- =========================================================================
--
-- Connection Pooling:
--   Supabase uses PgBouncer in transaction mode by default (port 6543).
--   For 1000+ concurrent users, ensure:
--     - Use the pooled connection string (port 6543), not direct (5432)
--     - Default pool_size=15 is sufficient for most workloads
--     - Monitor via: SELECT * FROM pgbouncer.pools;
--
-- Performance Monitoring:
--   Enable pg_stat_statements for query analysis:
--     CREATE EXTENSION IF NOT EXISTS pg_stat_statements;
--   Key views:
--     - pg_stat_user_tables: table I/O stats (seq scans vs index scans)
--     - pg_stat_user_indexes: index usage frequency
--     - pg_stat_statements: slow query identification
--
-- Index Tuning:
--   All foreign keys are indexed. Composite indexes exist for common
--   query patterns (owner_id + updated_at DESC for project listing).
--   Partial indexes (WHERE clauses) reduce index size for sparse data.
--   GIN index on marketplace_items.tags for array containment queries.
--
-- Autovacuum:
--   PostgreSQL autovacuum handles MVCC cleanup. For high-write tables
--   (observability_events, ai_request_log), consider:
--     ALTER TABLE public.observability_events
--       SET (autovacuum_vacuum_scale_factor = 0.05);
--
-- Partitioning (future, if needed):
--   observability_events and audit_log are candidates for time-based
--   partitioning (PARTITION BY RANGE on created_at) if they exceed
--   millions of rows.
