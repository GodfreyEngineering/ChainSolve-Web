-- ============================================================
-- ChainSolve — Baseline Schema (consolidated)
--
-- Single migration for a fresh Supabase project.
-- Supersedes iterative migrations 0001–0054 (archived).
--
...
--
-- See docs/DEV/SUPABASE_BOOTSTRAP.md for full setup steps.
-- ============================================================
...
-- If the enum already exists (e.g. partial migration), ensure values are present.
...
-- ── 2a. profiles ─────────────────────────────────────────────────────────────
-- One row per auth.users entry, auto-created by trigger.
...
CREATE TABLE IF NOT EXISTS public.profiles (
  ...
  -- Role flags (service_role only — users cannot self-promote)
...
CREATE TABLE IF NOT EXISTS public.organizations (
  ...
  -- Policy flags (enterprise controls)
...
-- Alternate names flagged by Supabase advisor (point at same logic)
...
-- ── 3e. Marketplace counter functions ────────────────────────────────────────
...
-- ── 3f. Marketplace official flag auto-set ───────────────────────────────────
...
-- 6. RLS POLICIES
--
-- Naming: {table}_{action}_own  (canonical form from 0011)
-- Predicates use (select auth.uid()) for InitPlan optimization.
-- ═══════════════════════════════════════════════════════════════════════════════
...
-- ── 6i. csp_reports ──────────────────────────────────────────────────────────
-- Only service_role inserts (CSP reports carry no auth token).
-- Authenticated INSERT provided for future client-side JS error reporting.
...
-- Only verified authors can publish items.
...
-- Consolidated UPDATE: authors + moderators.
...
-- ── 6r. audit_log ────────────────────────────────────────────────────────────
-- Consolidated SELECT: own events + org admin events.
...
-- ai_org_policies: org members can read, owners can update
...
-- ai_usage_monthly: users read own; service_role writes
CREATE POLICY ai_usage_monthly_select ON public.ai_usage_monthly
  FOR SELECT TO authenticated
  USING (owner_id = (select auth.uid()));

-- ai_request_log: users read own; service_role writes
...
-- Only service_role can insert/update. Users can read own.
...
-- ── projects bucket (gated by user_can_write_projects) ───────────────────────
...
-- ── uploads bucket (gated by user_has_active_plan) ───────────────────────────
