...
| Rename… | Prompts for a new label, updates node label on confirm |
| Duplicate | Copy node at +24 px / +24 px offset |
| Lock position | Set `node.draggable = false`; shows 🔒 in node header |
| Unlock position | Restore `node.draggable` to default (draggable) |
...
| Item | Action |
| --- | --- |
| Add block here | Opens QuickAddPalette at cursor position |
| Fit view | Fit all nodes into the viewport |

### QuickAddPalette (`Add block here`)

- Floating panel anchored at cursor, clamped to stay on-screen.
- Auto-focuses a search input; all blocks are listed, filtered as you type.
- Keyboard: `↑` `↓` move selection, `Enter` adds the highlighted block, `ESC` dismisses.
- Clicking outside (transparent overlay) also dismisses.
- Adds the block at the exact canvas position where the user right-clicked.
